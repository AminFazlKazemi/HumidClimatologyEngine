...
def main():
    ...
    # ===== پچ: گزارش وضعیت چک‌پوینت‌های سالانه =====
    completed_years = [y for y in years if is_year_complete(y)]
    missing_years = [y for y in years if not is_year_complete(y)]
    logger.info("="*80)
    logger.info("YEAR CHECKPOINT STATUS")
    logger.info(f"Completed: {len(completed_years)}/{len(years)}")
    if missing_years:
        logger.info(f"Missing/invalid years: {missing_years}")
    else:
        logger.info("All years are already completed.")
    logger.info("="*80)
    ...
    if remaining:
        ...
        logger.info("No year processing needed; all years are complete.")
    ...
    # ===== پچ: گزارش وضعیت چک‌پوینت‌های روزانه =====
    completed_days = [d for d in range(1, DOY_COUNT+1) if valid_day_checkpoint(d)]
    missing_days = [d for d in range(1, DOY_COUNT+1) if not valid_day_checkpoint(d)]
    logger.info("="*80)
    logger.info("DAILY CHECKPOINT STATUS")
    logger.info(f"Completed: {len(completed_days)}/{DOY_COUNT}")
    if missing_days:
        logger.info(f"Missing/invalid days: {missing_days[:10]}{'...' if len(missing_days)>10 else ''}")
    else:
        logger.info("All daily checkpoints are already completed.")
    logger.info("="*80)

    log_progress("STAGE", phase="DAILY MONTE CARLO")
    # ---- Daily MC
    if len(completed_days) == DOY_COUNT:
        logger.info("All days completed; skipping Monte Carlo phase.")
    else:
        logger.info("Running daily Monte Carlo for missing days...")
