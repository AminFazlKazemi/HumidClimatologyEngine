...
#os.environ["PROCESS_YEARS"] = "[1981,1982,1983,1984,1985,1986,1987,1988,1989,1990]"
#os.environ["PROCESS_YEARS"] = "[1991,1992,1993,1994,1995,1996,1997,1998,1999,2000]"
os.environ["PROCESS_YEARS"] = "[2001,2002,2003,2004,2005,2006,2007,2008,2009,2010]"
#os.environ["PROCESS_YEARS"] = "[2011,2012,2013,2014,2015,2016,2017,2018,2019,2020]"
...
def process_year_empirical(year: int) -> tuple[int, Optional[Path]]:
    ...
    try:
        ...
        total_slots_per_year = 365  # slot 59 is reserved; Feb 28/29 are pooled.
        ...
        for month in range(1, 13):
            ...
            with open_dataset(t_idx[month]) as ds_t, open_dataset(d_idx[month]) as ds_d, open_dataset(p_idx[month]) as ds_p:
                ds_t = sort_dataset(ds_t); ds_d = sort_dataset(ds_d); ds_p = sort_dataset(ds_p)
                validate_grids_and_axes(ds_t, ds_d, ds_p, year, month)
                tu = ds_t["t2m"].attrs.get("units"); du = ds_d["d2m"].attrs.get("units"); pu = ds_p["sp"].attrs.get("units")
                ...
                month_slots = sorted(set(get_clim_doy(int(d), year) for d in native_doys))

                for cdoy in month_slots:
                    if cdoy < 1 or cdoy > DOY_COUNT or cdoy == 59:
                        continue
                    slot_mask = np.asarray([get_clim_doy(int(d), year) == cdoy for d in native_doys], dtype=bool)
                    slot_dates = sorted(set(date_texts[slot_mask].tolist()))
                    time_idx = np.flatnonzero(slot_mask)
                    ...
                    for yci, j0 in enumerate(range(0, ny, CHUNK_LAT)):
                        ...
                        for xci, i0 in enumerate(range(0, nx, CHUNK_LON)):
                            ...
                            for ti in time_idx:
                                T = convert_temperature(ds_t["t2m"].isel(time=int(ti), latitude=slice(j0, j1), longitude=slice(i0, i1)).values, tu, "t2m").reshape(-1)
                                Td = convert_temperature(ds_d["d2m"].isel(time=int(ti), latitude=slice(j0, j1), longitude=slice(i0, i1)).values, du, "d2m").reshape(-1)
                            ...
                            if slot_completed % PROGRESS_LOG_EVERY_CHUNKS == 0 or slot_completed == n_y_chunks * n_x_chunks:
                                _log_progress(year, completed_units, total_units, time.time() - t0, f"DOY {cdoy:03d} chunks {slot_completed}/{n_y_chunks*n_x_chunks}")
                    ...
                    if slot_completed == n_y_chunks * n_x_chunks:
                        ...
                        logger.info(
                            ...
                            year, cdoy, 100.0 * completed_units / total_units, total_units - completed_units,
                        )

        ds_ckpt.sync()
        import calendar as _calendar
        expected_dates = {f"{year}-{month:02d}-{day:02d}" for month in range(1, 13) for day in range(1, _calendar.monthrange(year, month)[1] + 1)}
        if completed_dates != expected_dates:
            missing = sorted(expected_dates - completed_dates)
            raise RuntimeError(f"Year {year} incomplete; missing {len(missing)} native dates (first={missing[:3]})")
        if completed_units != total_units:
            raise RuntimeError(f"Year {year} incomplete checkpoint: {completed_units}/{total_units} spatial-day units committed")

        ds_ckpt.close(); ds_ckpt = None
        ...
        logger.info("POWER-FAILURE SAFE COMMIT | Year %04d | 100.00%% | %d/%d units | remaining 0 | elapsed %.1fs", year, completed_units, total_units, time.time() - t0)
        return year, final_path
    except Exception:
        logger.exception("Year %d failed; disk checkpoint is retained for restart", year)
        try:
            if ds_ckpt is not None:
                ds_ckpt.sync(); ds_ckpt.close()
        except Exception:
            pass
        return year, None
...
def main() -> None:
    ...
    # --- فیلتر سال‌ها از طریق متغیر محیطی PROCESS_YEARS ---
    import ast
    ...
    grand_total = per_year_units * len(years)   # فقط برای سال‌های انتخاب‌شده
    ...
    # =====================================================================
    # پردازش ترتیبی سال‌های ناقص (ایمن در برابر خطاهای HDF)
    # =====================================================================
    if remaining_years:
    ...
    # توجه: finalize_all فقط روی سال‌های انتخاب‌شده اجرا می‌شود.
    # پس از اتمام همه‌ی سال‌ها، یک بار بدون PROCESS_YEARS برنامه را اجرا کنید تا فایل‌های نهایی کامل تولید شوند.
